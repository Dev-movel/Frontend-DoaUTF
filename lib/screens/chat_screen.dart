...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _inicializar() async {
    ...
    _sub = channel.stream.listen(
      (data) {
        ...
        // Remove o placeholder local se existir e adiciona a confirmada
        setState(() {
  ...
  Future<void> _enviar() async {
    ...
    // Optimistic update: mostra a mensagem antes da confirmação do servidor
      ...
      body: Column(
        children: [
          if (!_carregando && !_conectado)
            _BannerDesconectado(
              onReconectar: () async {
                final ch = await ChatService.instance
                    .conectar(widget.solicitacaoId);
                if (!mounted) return;
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      bottom: PreferredSize(
        ...
        child: Divider(
            height: 1, color: AppColors.outline.withOpacity(0.2)),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
              child:
                  Divider(color: AppColors.outline.withOpacity(0.3), height: 1)),
          Padding(
